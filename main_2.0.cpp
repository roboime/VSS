...
int main(int argc, char** argv){
	...
	string ip = argc>1?argv[1]:"192.168.0.101";
    cout << "ip: " << ip;
    inet_aton(ip.c_str(), &addrF.sin_addr);
    addrF.sin_port = htons(10010);
   	int sockF = socket(PF_INET, SOCK_DGRAM, 0);

    // Envio dos pacotes dos inimigos
    struct sockaddr_in addrE;
    addrE.sin_family = AF_INET;
    inet_aton(ip.c_str(), &addrE.sin_addr);
    addrE.sin_port = htons(10015);
...
*/
        int debug = recvfrom(fd, &bufReceiver, 18, 0, (struct sockaddr *)&remaddr, &addrlen);
   //     viewerReceiver.ParseFromString(bufReceiver);
	//viewerReceiver.has_friend2veldir();	
	cout << debug <<endl;
        // cout << viewerReceiver.friend0veldir() << endl;
	...
	if(debug==-1){
		sendVel[0] = sendVel_old[0];
        	sendVel[1] = sendVel_old[1];
        	sendVel[2] = sendVel_old[2];
        	sendVel[3] = sendVel_old[3];
        	sendVel[4] = sendVel_old[4];
        	sendVel[5] = sendVel_old[5];       	
	}else{
//		sendVel[0] = viewerReceiver.friend0veldir() - 256;
  //      	sendVel[1] = viewerReceiver.friend1veldir() - 256;
	...
	sendVel_old[0] = sendVel[0];
        sendVel_old[1] = sendVel[1];
        sendVel_old[2] = sendVel[2];
        sendVel_old[3] = sendVel[3];
        sendVel_old[4] = sendVel[4];
        sendVel_old[5] = sendVel[5];
...
//	aux[5] = vel[5];
  ...
  Command command;
	...
	for(int i = 0 ; i < 3 ; i++){                           // todos os robôs vão girar
      ...
      commandSender->sendCommand(command);// Não bloqueante, envia os comandos para o VSS-Simulator
